...
\begin{quote}
\itshape
Following from the dual nature of the Galactic stellar halo,
accreted and in-situ stars overlap in the chemo-dynamical space
making the separation of these two stellar populations a non-trivial task which, however,
comprises the basis of every Galactic archaeology study.
This chapter addresses the problem of selecting accreted stars from the in-situ background by
leveraging the information on the properties of accreted and in-sity star particles in
the \texttt{ARTEMIS} simulations.
Comparing a range of machine learning algorithms and input features, I identify a classifier that returns a
purer accreted sample than selection cuts, establishing which observable stellar properties
are more important for the discrimination.
Clean accreted samples are the necessary input to the next stage of
the Galactic archaeology workflow, i.e. grouping those stars into the progenitor systems where they
formed, which is the subject of Chapter~\ref{chap:chapter3}.
\end{quote}
